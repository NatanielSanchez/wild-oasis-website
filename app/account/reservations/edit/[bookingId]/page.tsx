...
export default async function Page({ params }: { params: { bookingId: string } }) {
  ...
  if (!booking || !booking.cabinId || !booking.numGuests) notFound();
  const { id, numGuests, observations } = booking;

  const { maxCapacity } = await getCabin(booking.cabinId);
  if (!maxCapacity)
    throw new Error(
      "Booking with ID " + params.bookingId + " has no cabin, or cabin has no max capacity!",
    );

  return (
    <div>
      <h2 className="mb-7 text-2xl font-semibold text-accent-400">Edit Reservation #{id}</h2>
