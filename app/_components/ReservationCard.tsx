...
import { BookingReservation } from "../_lib/types";
...
function ReservationCard({
  ...
  // database was setup weird, so alot of things that would never be null are returned as possibly null, so i just force it with !
  const { id, startDate, endDate, numNights, totalPrice, numGuests, created_at, cabins } = booking;

  return (
    <div className="flex border border-primary-800">
      <div className="relative aspect-square h-32">
        <Image
          src={cabins?.image || "/404.png"}
          fill
